...
class BertMultiTaskForCTCF(BertPreTrainedModel):
    def __init__(self, config, lambda_weight=0.15, model_name="zhihan1996/DNA_bert_3"):
        ...
        self.token_classifier = nn.Linear(config.hidden_size, 1)  # removed Sigmoid
        self.sequence_classifier = nn.Linear(config.hidden_size, 1)  # removed Sigmoid
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, sequence_labels=None, class_type=None):
        ...
        sequence_output = outputs.last_hidden_state[:, 1:-1, :]  # remove [CLS] and [SEP] from token outputs
