...
MODEL_SAVE_PATH = "../checkpoints/best_model_v3.pt"
...
for epoch in range(MAX_EPOCHS):
    ...
    for step, batch in enumerate(pbar):
        ...
        labels = batch["labels"][:, 1:-1].to(device)  # match token_logits shape
        ...
        if (step + 1) % EVAL_FREQ == 0:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    true_seq = (labels.sum(dim=1).cpu().numpy() > 0).astype(int)
